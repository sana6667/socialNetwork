...
resource "aws_eks_access_entry" "github_role_access" {
  cluster_name  = aws_eks_cluster.eks_priv_endpoint.name
  principal_arn = "arn:aws:iam::336670698191:role/github-terraform-role"
  type          = "STANDARD"
...
resource "aws_eks_access_policy_association" "github_role_admin" {
  ...
  depends_on = [
    aws_eks_access_entry.github_role_access,
    aws_eks_cluster.eks_priv_endpoint
  ]
}
